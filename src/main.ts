...
async function saveDeck(event: SubmitEvent): Promise<void> {
  ...
  const prompts = form.elements.namedItem('prompts') as HTMLTextAreaElement;
  const promptError = document.querySelector<HTMLElement>('#deck-prompts-error');
  if (!lines.length) {
    prompts.setAttribute('aria-invalid', 'true');
    if (promptError) { promptError.textContent = 'Add at least one prompt.'; promptError.classList.remove('hidden'); }
    prompts.focus();
    return;
  }
  prompts.removeAttribute('aria-invalid');
  if (promptError) { promptError.textContent = ''; promptError.classList.add('hidden'); }
...
async function startSession(deckId: string, record: boolean): Promise<void> {
  ...
  if (record) {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === 'undefined') { recordingNotice = 'Recording is not available in this browser. This session will not save a recording.'; record = false; }
    else {
      try { stream = await navigator.mediaDevices.getUserMedia({ audio: { echoCancellation: true } }); }
      catch { recordingNotice = 'Microphone access was not granted. This session will not save a recording. Check browser permissions and start a new recorded rehearsal when ready.'; record = false; }
    }
  }
  session = { deck: structuredClone(deck), index: 0, phase: 'prompt', record, stream, chunks: [], remainingMs: settings.gapSeconds * 1000, paused: false, recordingNotice };
  render();
